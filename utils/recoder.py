...
class TxtRecorder(object):
    ...
    def record_str(self, data):
        with open(self.log_path, encoding="utf-8", mode="a") as f:
            f.write(data)
            f.write("\n")
...
class Printer(object):
    def __init__(self, default_local_rank: int = 0):
        self.default_local_rank = default_local_rank

    def print_on_device(self, out_str: str, total_length: int = 80, local_rank: int = 0):
        if local_rank == self.default_local_rank:
            self.__construct_print(out_str, total_length)

    def print_derectly(self, out_str: str, total_length: int = 80):
        self.__construct_print(out_str, total_length)

    def __construct_print(self, out_str: str, total_length: int = 80):
